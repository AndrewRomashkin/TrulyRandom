...
public partial class NistTests
{
    ...
    public class FullTestResult
    {
        ...
        /// </summary>
        public Dictionary<TestType, TestResult> FailedTests => TestResults.Where(x => !x.Value.Success).ToDictionary(dict => dict.Key, dict => dict.Value);
        /// <summary>
        /// Test had failed only due to insufficient number of cycles (tests <see cref="TestType.RandomExcursions"/> and <see cref="TestType.RandomExcursions"/>).
        /// Try to increase sequence size to reduce the number of such failures.
        /// </summary>
        public bool InsufficientCycles => FailedTests.Count > 0 && FailedTests.Values.All(x => x.Result == ResultType.IncufficientCycles);
        /// <summary>
        ...
        /// </summary>
        public Dictionary<TestType, TestResult> TestResults { get; private set; } = new Dictionary<TestType, TestResult>();
        /// <summary>
        /// Human-readable report about the test.
        /// </summary>
        public string Report { get; private set; } = "";

        internal FullTestResult(Dictionary<TestType, TestResult> testResults, double proportionOfTestsAllowedToFail, int time)
        {
