...
public partial class NistTests
{
    ...
    /// Recent test results.
    /// </summary>
    private readonly List<Dictionary<TestType, double[]>> testHistory = new();
    ...
    /// <returns>A <see cref="FullTestResult"/> object containing information abouth the results of aech test and an overlall conclusion.</returns>
    public FullTestResult Perform(byte[] data, TestType testsToPerform)
    {
    ...
    /// <param name="testsToPerform">Tests to perform.</param>
    /// <returns>A <see cref="FullTestResult"/> object containing information abouth the results of aech test and an overlall conclusion.</returns>
    public FullTestResult Perform(BitArray data, TestType testsToPerform)
    {
        Stopwatch sw = new();
        sw.Start();
        Dictionary<TestType, TestResult> results = new();
        ...
        if (testsToPerform.HasFlag(TestType.DiscreteFourierTransform))
        {
            actions.Add(() =>
            {
                ...
                lock (testHistory)
                {
                    lock (results)
                    {
                        results.Add(TestType.DiscreteFourierTransform, new TestResult(rawTestResult, Parameters, TestType.DiscreteFourierTransform, testHistory.Where(x => x.ContainsKey(TestType.DiscreteFourierTransform)).Select(x => x[TestType.DiscreteFourierTransform]).ToArray()));
                    }
                }
            });
        }

        if (testsToPerform.HasFlag(TestType.NonOverlappingTemplateMatchings))
        {
            actions.Add(() =>
            {
                Thread.CurrentThread.Priority = ThreadPriority.Lowest;
                ...
                lock (testHistory)
                {
                    lock (results)
                    {
                        results.Add(TestType.NonOverlappingTemplateMatchings, new TestResult(rawTestResult, Parameters, TestType.NonOverlappingTemplateMatchings, testHistory.Where(x => x.ContainsKey(TestType.NonOverlappingTemplateMatchings)).Select(x => x[TestType.NonOverlappingTemplateMatchings]).ToArray()));
                    }
                }
            });
        }

        if (testsToPerform.HasFlag(TestType.LinearComplexity))
        {
            actions.Add(() =>
            {
                Thread.CurrentThread.Priority = ThreadPriority.Lowest;
                ...
                lock (testHistory)
                {
                    lock (results)
                    {
                        results.Add(TestType.LinearComplexity, new TestResult(rawTestResult, Parameters, TestType.LinearComplexity, testHistory.Where(x => x.ContainsKey(TestType.LinearComplexity)).Select(x => x[TestType.LinearComplexity]).ToArray()));
                    }
                }
            });
        }

        if ((testsToPerform & ~TestType.DiscreteFourierTransform & ~TestType.NonOverlappingTemplateMatchings & ~TestType.LinearComplexity) != TestType.None)
        {
            actions.Add(() =>
            {
                Thread.CurrentThread.Priority = ThreadPriority.Lowest;
                SingleTestResult rawTestResult;
                foreach (TestType test in (testsToPerform & ~TestType.DiscreteFourierTransform & ~TestType.NonOverlappingTemplateMatchings
                    & ~TestType.LinearComplexity).EnumerateFlags())
                {
                    switch (test)
                    {
                        case TestType.Frequency:
                            rawTestResult = Frequency(data);
                            break;
                        case TestType.BlockFrequency:
                            rawTestResult = BlockFrequency(data, Parameters.BlockFrequency.BlockLength);
                            break;
                        case TestType.Runs:
                            rawTestResult = Runs(data);
                            break;
                        case TestType.LongestRunOfOnes:
                            rawTestResult = LongestRunOfOnes(data);
                            break;
                        case TestType.BinaryMatrixRank:
                            rawTestResult = BinaryMatrixRank(data, Parameters.BinaryMatrixRank.MatrixSize);
                            break;
                        case TestType.OverlappingTemplateMatchings:
                            ...
                            break;
                        case TestType.MaurersUniversal:
        ...
        Dictionary<TestType, double[]> pvalues = new();

        foreach (TestType test in testsToPerform.EnumerateFlags())
        {
    ...
    public static class MinSequenceSizes
    {
        ...
        /// Note that for this test it is recommended to keep block size as big as possible to avoid getting an <see cref="ResultType.IncufficientCycles"/> resut.
        /// </summary>
